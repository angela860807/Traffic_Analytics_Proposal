...
# ── 설정 ───────────────────────────────────────────────────────
N_CAMERAS = 3                  # 카메라 수
NORMAL_DAYS = 14               # 정상 데이터 기간 (일) → 기준선용
...
def apply_fault(base: dict, fault_type: str, progress: float) -> dict:
    ...
    if fault_type == "CAMERA_OFFLINE":
        ...
        # 교통량 감소 - 장비 지표는 정상 (오탐 방지 시나리오)
        # fps/latency/cpu 등 장비 지표는 그대로 유지
        s["is_fault"] = 0   # 장비 이상 아님 → 라벨 0
...
def generate_all_data():
    ...
    for scenario_idx, fault_type in enumerate(fault_scenarios):
        ...
        # 장애 구간 (4시간, 점진적 악화)
        fault_duration = 240  # 분
        for i in range(fault_duration):
            progress = i / fault_duration   # 0.0 → 1.0
    ...
    df_normal.to_csv("data/normal_samples.csv", index=False)
    df_fault.to_csv("data/fault_samples.csv", index=False)
    df_all.to_csv("data/all_samples.csv", index=False)

    print(f"\n✅ 저장 완료")
    print(f"   data/normal_samples.csv  : {len(df_normal):,}행")
    print(f"   data/fault_samples.csv   : {len(df_fault):,}행")
    print(f"   data/all_samples.csv     : {len(df_all):,}행")
