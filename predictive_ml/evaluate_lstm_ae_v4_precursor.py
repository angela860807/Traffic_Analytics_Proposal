...
MODEL_PATH     = "model/lstm_ae_v3_thr999.pt"
SCALER_PATH    = "model/scaler_v3.pkl"
THRESHOLD_PATH = "model/threshold_v3_thr999.json"
FAULT_DATA     = "data/fault_samples4.csv"
...
def measure_lead_time(model, scaler, warning_thr, df_fault):
    ...
    df_lead.to_csv("results/lead_time_report_v4.csv", index=False)
    print(f"\n  저장 완료: results/lead_time_report_v4.csv")
    return df_lead
...
def analyze_by_scenario(model, scaler, warning_thr, df_fault):
    ...
    df_scenario.to_csv("results/scenario_metrics_v4.csv", index=False)
    print(f"\n  저장 완료: results/scenario_metrics_v4.csv")
    return df_scenario
...
def save_summary(df_lead, df_scenario, warning_thr, critical_thr):
    ...
    with open("results/evaluation_summary_v4.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)
    print(f"\n  저장 완료: results/evaluation_summary_v4.json")
    return summary
...
def main():
    ...
    print("═" * 50)
    print("\n결과 파일:")
    print("  results/lead_time_report_v4.csv")
    print("  results/scenario_metrics_v4.csv")
    print("  results/evaluation_summary_v4.json")
