...
- [x] `memoryUsagePct`
- [x] `diskUsagePct`
- [x] `networkRttMs` 또는 측정 불가 시 `null`
- [x] `lastFrameAt`
- [x] `qualityStatus`
- [x] `dataSource`
...
- [x] `POST /internal/v1/camera-health-samples` client 구현
- [x] `X-Internal-Api-Key`, `X-Request-Id` 전달
- [x] timeout 3초
- [x] 지수 backoff retry 최대 3회
- [x] 제한된 로컬 재전송 큐
- [x] 재전송 시 원래 `sampledAt`과 `idempotencyKey` 유지
- [x] 큐 포화 시 drop metric 기록
- [x] 영상 추론 thread와 전송 thread 분리
...
- [x] Pydantic 요청·응답 schema
- [x] `predictive_ml.detect_rules()` 호출
- [x] AI 결과의 Enum과 API 계약 검증
- [x] detector version을 응답에 포함
...
- [x] 최근 샘플·기준선·교통 맥락 schema
- [x] `predictive_ml.detect_degradation()` 호출
- [x] `READY`, `LEARNING` 기준선 상태 전달
- [x] trend와 evidence 응답 변환
...
- [x] 서비스 상태
- [x] 설치된 AI 패키지 버전
- [x] 활성 detector 이름·버전·방식
- [x] AI 패키지 import 실패 상태
...
- [x] 로컬 editable install 또는 wheel 설치 방식 확정
- [ ] Python 버전과 의존성 충돌 확인
- [x] 입력 객체를 dict 임의 조작 없이 명시적으로 변환
- [x] AI 패키지 예외를 `INTERNAL_DETECTOR_UNAVAILABLE`로 매핑
- [x] 후보가 없을 때 정상 빈 배열 응답
- [x] package version과 detector version 구분
- [x] LSTM AutoEncoder `.pt`, scaler, threshold metadata를 시작 시 1회 로드
- [x] artifact SHA-256과 feature schema version 검증
- [x] feature 순서·누락 feature 불일치 시 추론 거부
- [x] 모델 결과를 `shadowCandidates`로만 반환
- [x] 기준선 상태가 `LEARNING`이면 모델 추론을 건너뜀
...
- [x] 내부 API key 검증
- [x] Pydantic validation error 표준화
- [x] requestId, cameraId, detectorVersion 구조화 로그
- [x] 요청 body 크기 제한
- [x] endpoint별 처리 시간 metric
- [x] AI adapter 호출 시간 metric
- [x] Spring 전송 성공·재시도·drop metric
- [x] 부분 후보 반환 없이 명시적 실패 응답

## 8. 테스트

파트 테스트는 다음 3개만 수행하고 전체 E2E와 성능 검증은 최종 병합 담당이 수행한다.

- [x] 상태 지표 1분 집계·결측값·idempotency key 단위 테스트
- [x] AI artifact 로딩·feature schema 검증·SHADOW 응답 변환 단위 테스트
- [x] AI fixture와 Spring Boot mock을 사용한 두 탐지 endpoint 인계 스모크 테스트
...
- [x] Spring URL과 내부 API key 환경변수화
- [x] metric window와 retry queue 크기 환경변수화
- [x] `/internal/v1/anomaly-detection/health` health check
- [x] CPU·메모리 제한 적용
- [x] AI 패키지 버전 시작 로그 출력
...
- [x] 두 탐지 API와 health API가 계약대로 동작한다.
- [x] FastAPI 내부에 detector 계산식이 중복되지 않는다.
- [x] AI 패키지 버전과 detector version이 노출된다.
- [ ] LSTM AutoEncoder가 SHADOW 모드로 로드되고 운영 candidate와 분리된다.
- [x] 최소 자동 테스트 3개와 Spring·AI 인계 계약이 통과한다.
- [x] 상태 수집 실패가 기존 영상 분석을 중단시키지 않는다.
