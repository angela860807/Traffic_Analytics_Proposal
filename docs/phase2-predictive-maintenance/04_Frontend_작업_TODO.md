...
|---|---|
| 구현 소유 | Frontend 파트 |
| 공개 API 계약 협업 | Spring Boot 파트 |

Vue 3 Composition API를 사용하며 view, component, composable, store, router, API client를 분리한다.

소유 범위는 `trafficAS-b/src/**`의 예지보전 화면과 API client다. AI·DB 산출물을 직접 읽지 않고 Spring Boot 공개 API만 사용한다.
...
| 6/14~6/15 | 티켓·정책 화면 |
| 6/16 | 권한·오류·반응형·접근성 |
| 6/17 | 최소 단위 테스트·mock API 인계 점검 |
| 6/18 | UI 최종 확인·발표 데이터 점검 |
| 6/19 | 버퍼·최종 시연 |
