...
본 문서는 ERD, API 계약서, Frontend, Spring Boot, FastAPI, AI 모델, DB 작업의 최상위 기준이다. 계약을 변경할 때는 8개 문서를 함께 수정한다.
...
- 정상 시계열로 학습한 LSTM AutoEncoder의 SHADOW 모드 비교 추론
...
LSTM AutoEncoder는 학습·평가·FastAPI 적재까지 수행하지만 `SHADOW` 모드로만 동작한다. Isolation Forest는 일정이 허용될 때 비교 baseline으로만 실험한다.
...
| `ModelPredictionLog` | `cameraId + detectorVersion + evaluatedAt` | 5분 SHADOW 추론 |
| `MaintenanceTicket` | 운영자가 처리하는 작업 하나 | 업무 |

원천 이벤트, 집계 샘플, 탐지 결과, 정비 결과를 같은 테이블에 저장하지 않는다.

MVP는 카메라 1대와 edge processor 1대가 대응한다고 가정한다. 여러 카메라가 하나의 processor를 공유하는 배포는 별도 `PROCESSOR` 자산 모델이 필요하므로 고도화 범위로 둔다.

### 5-2. 시간

- DB는 PostgreSQL 16을 기준으로 하며 시간 컬럼은 `TIMESTAMPTZ`, Java는 `OffsetDateTime`을 사용한다.
...
- 데모에서는 동일 데이터셋 manifest의 `SIMULATED` 정상 샘플 30개 이상을 기준선으로 사용하고 이후 `FAULT_INJECTED` 구간을 평가할 수 있다.
- 데모 탐지 결과와 `REAL` 운영 평가는 화면·보고서에서 분리한다.
...
- PostgreSQL upsert는 명시된 unique constraint를 대상으로 `INSERT ... ON CONFLICT`를 사용한다.
...
- 최소 30개 표본이 필요하다.
- DB 조회로 중앙값과 MAD를 계산하고, 탐지에 사용한 값은 이벤트 근거에 snapshot으로 저장한다.
...
| 예측 구간 | 향후 10분 |
| 신뢰도 | 시간과 EWMA 값의 Spearman 상관계수 절댓값 |
...
| FR-ML-002 | 정상 시계열로 LSTM AutoEncoder를 학습하고 SHADOW 추론한다. | `.pt` 모델, scaler, 학습 manifest, threshold, 평가 결과가 재현된다. |
| FR-ML-003 | SHADOW 예측은 별도 로그로 저장한다. | SHADOW 결과만으로 이상 이벤트나 티켓이 생성되지 않는다. |
...
| NFR-DATA-002 | 신규 스키마는 기존 규칙의 번호형 PostgreSQL migration SQL로만 변경한다. |
| NFR-TEST-001 | 각 파트는 핵심 단위 테스트 2개와 인계 스모크 테스트 1개까지만 자동화하고, 전체 E2E는 최종 병합 담당이 수행한다. |
...
LSTM AutoEncoder Detector (SHADOW)
```

앞의 네 detector는 학습 모델이 아니다. LSTM AutoEncoder만 정상 시계열 sequence를 학습하며 운영 detector와 동일 입력에서 비교 추론한다. Isolation Forest는 선택 비교 baseline이고, 실제 고장 라벨이 충분히 쌓이면 LightGBM/XGBoost를 검토한다.

LSTM AutoEncoder는 다변량 시계열 sequence의 복합 이상 패턴을 탐지하고, 향후 10분 임계치 도달 예측은 Trend Projection Detector가 담당한다.

### 11-2. LSTM AutoEncoder 학습 계약

| 항목 | 정책 |
|---|---|
| 학습 단위 | `cameraId + sequenceEndAt` |
| 입력 sequence | 직전 60분 × 8개 feature |
| stride | 5분 |
| 학습 데이터 | 정상 14일, `COMPLETE`, 비보간 sequence |
| 모델 | PyTorch LSTM AutoEncoder |
| 전처리 | train 정상 구간에서만 fit한 scaler |
| 배포 형식 | `.pt` state dict + scaler + JSON metadata |
| 운영 모드 | `SHADOW` |

- 입력 feature는 FPS, frame drop, latency, blur, OCR 실패율, CPU, memory, network RTT의 8개다.
- 시뮬레이션 정상 데이터 `60,480`행과 장애 평가 데이터 `4,320`행을 목표 데이터셋으로 한다.
- 카메라별 차이를 줄이도록 train 정상 데이터에만 scaler를 fit한다.
- 정상 데이터는 시간순 train 70%, validation 15%, test 15%로 분리하고 경계마다 60분 purge gap을 둔다.
- 장애 주입 데이터는 train에서 제외하고 test 평가에만 사용한다.
- 정상 validation 재구성 오차의 99백분위수를 WARNING, 99.9백분위수를 CRITICAL 초기값으로 사용한다.
- validation에서 카메라 1대·1일당 오탐 1건을 초과하면 threshold를 상향 조정한다.
- test threshold는 고정한 뒤 1회 평가한다.
- 학습 가능한 정상 sequence가 1,000개 미만이면 모델은 `EXPERIMENTAL` 상태로 남기고 FastAPI 운영 이미지에 적재하지 않는다.
- 카메라 기준선이 `LEARNING`이면 해당 카메라의 학습 모델 추론도 실행하지 않는다.
- artifact에는 모델, scaler, feature 순서, WARNING·CRITICAL 재구성 오차 threshold를 함께 저장한다.
...
- LSTM AutoEncoder와 Rule·z-score·추세 결과 비교
- SHADOW 예측의 일치·불일치 사례 분석
...
| 담당 파트 | 핵심 책임 | 주요 인계 산출물 | 업무 비중 |
|---|---|---|---:|
| AI 모델 파트 | 시뮬레이션 데이터, Rule·z-score·추세 로직, LSTM AE 학습·평가 | `predictive_ml` wheel, `.pt`, scaler, threshold, metrics, fixture | 27% |
| Spring Boot 파트 | 데이터 저장 orchestration, 이벤트·티켓·SLA, 공개 API | DTO·Entity·Service·Repository, 최소 자동 테스트 | 23% |
| DB 파트 | PostgreSQL schema, 기준선·학습 export SQL, seed·성능·권한 | migration, query, seed, validation, 성능 결과 | 18% |
| FastAPI 파트 | 1분 지표 수집, 내부 API, AI artifact 적재·추론 adapter | Pydantic 계약, client, adapter, Docker·최소 테스트 | 17% |
| Frontend 파트 | 관제 대시보드, 이상 근거, SHADOW 비교, 정비 업무 UI | Vue 화면·API client·route·최소 테스트 | 15% |

각 파트의 테스트 책임은 최대 3개로 제한한다. 최종 병합 담당이 Docker 환경의 전체 E2E와 성능 확인을 수행하며, 파트 담당자는 실패 원인 수정에만 참여한다.

### 12-1. 파일 소유권

| 파트 | 단독 소유 경로 | 병합 계약 |
|---|---|---|
| Frontend | `trafficAS-b/src/**` 중 예지보전 화면·API client | Spring 공개 API |
| Spring Boot | `backend/traffic/src/main/java/**`, 애플리케이션 설정·빌드 파일 | Frontend 공개 API, FastAPI 내부 API, DB schema |
| FastAPI | `fastapi-server/app/**` | Spring 내부 API, AI Python package |
| AI 모델 | `predictive_ml/**` | 순수 Python detector 입력·출력 |
| DB | `backend/traffic/src/main/resources/db/migration/**`, `db/**` | ERD, JPA mapping, 조회 성능 |

- AI 모델 파트는 `fastapi-server/app/**`를 수정하지 않는다.
- FastAPI 파트는 `predictive_ml/**`의 탐지 수식과 임계값을 수정하지 않는다.
- DB 파트는 Spring Java Entity·Repository를 수정하지 않는다.
- Spring Boot 파트는 migration SQL을 수정하지 않고 필요한 변경을 DB 파트에 schema 계약으로 요청한다.

### 12-2. 병합 순서

1. DB 파트가 migration·seed·학습 export 계약을 확정한다.
2. AI 모델 파트가 데이터 생성기와 detector 인터페이스를 확정한다.
3. FastAPI 파트가 AI wheel·artifact를 내부 API에 연결한다.
4. Spring Boot 파트가 수집·탐지 orchestration과 이벤트·티켓을 연결한다.
5. Frontend 파트가 확정된 공개 API로 화면을 통합한다.
6. 최종 병합 담당이 아래 3개 시나리오를 검증하고 실패 항목만 해당 파트에 전달한다.

### 12-3. 최종 병합 테스트

1. CCTV 장애 샘플이 ACTIVE detector를 거쳐 이상 이벤트·정비 티켓·관제 화면까지 연결된다.
2. LSTM AutoEncoder 결과가 `model_prediction_logs`와 화면에는 표시되지만 이벤트·티켓은 생성하지 않는다.
3. 교통량만 감소하고 CCTV 상태가 정상인 경우 정비 이벤트가 생성되지 않는다.

개별 장애 유형 9종의 탐지 정확도와 lead time은 AI 모델 평가 보고서로 확인하며, 전체 E2E를 9회 반복하지 않는다.
...
4. LSTM AutoEncoder `.pt` artifact와 학습·평가 manifest가 재현되고 SHADOW 결과가 저장된다.
5. SHADOW 결과만으로 이벤트나 티켓이 생성되지 않는다.
6. 교통 맥락은 CCTV 원인 검증에 사용되고 독립 정비 이벤트를 만들지 않는다.
7. 동일 이상 이벤트와 티켓이 중복 생성되지 않는다.
8. CRITICAL과 지속·예측 WARNING에 티켓이 자동 생성된다.
9. 운영자가 티켓을 배정하고 조치·종료할 수 있다.
10. 실제·시뮬레이션·장애 주입 데이터를 화면에서 구분한다.
11. 파트별 최소 자동 테스트와 최종 병합 테스트 3개가 통과한다.
12. 발표에서 오탐 수, lead time, MTTA, MTTR과 모델 비교 결과를 제시한다.
