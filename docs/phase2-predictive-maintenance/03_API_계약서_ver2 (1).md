...
상태 전이 권한:
- `OPEN → ASSIGNED`: `OPERATOR`, `ADMIN`
- `ASSIGNED → IN_PROGRESS`: `OPERATOR`, `MAINTAINER`, `ADMIN`
- `IN_PROGRESS → RESOLVED`: `OPERATOR`, `MAINTAINER`, `ADMIN`
- `RESOLVED → CLOSED`: `OPERATOR`, `ADMIN`
...
| 기존 교통·과속 조회 | O | O | O | O |
| 예지보전 대시보드 조회 | - | O | O | O |
| 이벤트 확인(acknowledge) | - | O | - | O |
| 이벤트 해결(resolve) | - | O | - | O |
| 이벤트 오탐 종료(dismiss) | - | O | - | O |
| 수동 정비 티켓 생성 | - | O | - | O |
| 티켓 배정 | - | O | - | O |
| 티켓 작업 상태·조치 변경 (ASSIGNED→IN_PROGRESS→RESOLVED) | - | O | O | O |
| 티켓 CLOSED 처리 | - | O | - | O |
| 정책 수정 | - | - | - | O |
