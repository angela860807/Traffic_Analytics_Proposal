...
- [ ] 명시 보류 상태를 프론트 localStorage가 아니라 백엔드 검증 이력으로 저장할지 결정
- [ ] `CLOSED`로 보낸 뒤 되돌리기 기능이 필요한지 결정
- [ ] `NOTIFIED`에서 `CLOSED`로 바로 가는 경우 사유를 필수로 받을지 결정
- [ ] 정상 속도 차량을 review 목록에 계속 표시할지, 과속 후보만 표시할지 최종 결정
- [ ] `stay_time`은 계속 미사용/NULL 정책으로 프론트에서 제외할지 재확인
