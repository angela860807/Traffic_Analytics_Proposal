      ...
      <section class="hero">
        <div class="hero-in">
          ...
          <div class="hero-right">
            <!-- public/illustrations/hero.svg 있으면 우선, 없으면 기존 PNG -->
            <img
              :src="heroImg"
              alt="시스템 구조 일러스트"
              class="hero-img"
            ...
            <template v-for="(a, i) in architecture" :key="a.num">
              <div class="arch-card">
                ...
                <div class="arch-body">
                  ...
                  <ul class="arch-list">
                    <li v-for="p in a.points" :key="p"><span class="dot">•</span>{{ p }}</li>
                  </ul>
                </div>
              </div>
        ...
        <div class="sw">
          ...
          <div class="lc-row">
            <template v-for="(s, i) in lifecycle" :key="s.title">
              <div class="lc-step">
                <div class="lc-icon"><component :is="s.lucide" :size="32" :stroke-width="1.8" /></div>
        ...
        <div class="sw">
          ...
          <div class="mod-row">
            <div v-for="m in modules" :key="m.num" class="mod-card">
              ...
              <div class="mod-graphic"><component :is="m.lucide" :size="72" :stroke-width="1.4" /></div>
        ...
        <div class="sw">
          ...
          <div class="deploy-row">
            <div class="deploy-box">
              <div class="deploy-h">연동 <span class="deploy-h-en">(Integration)</span></div>
              <div class="deploy-grid">
                <div v-for="ig in integrations" :key="ig.title" class="deploy-item">
                  <div class="deploy-icon"><component :is="ig.lucide" :size="26" :stroke-width="1.7" /></div>
            ...
            <div class="deploy-box">
              <div class="deploy-h">배포 <span class="deploy-h-en">(Deployment)</span></div>
              <div class="deploy-grid">
                <div v-for="d in deployments" :key="d.title" class="deploy-item">
                  <div class="deploy-icon"><component :is="d.lucide" :size="26" :stroke-width="1.7" /></div>
        ...
        <div class="sw">
          ...
          <div class="impact-row">
            <div v-for="k in kpis" :key="k.title" class="kpi-card">
              <div class="kpi-icon"><component :is="k.lucide" :size="28" :stroke-width="1.7" /></div>
...
import {
  Cctv, Camera, BrainCircuit, Database, LayoutDashboard,
  Car, ScanText, ArrowLeftRight, BellRing,
  Plug, Radio, Code2, FileDown,
  Cloud, Server, Network,
  Crosshair, Zap, Bot, TrendingUp, ShieldCheck, Infinity,
  ChevronRight,
} from "lucide-vue-next";

const { isDark } = useTheme();

/* 히어로 이미지 — public/illustrations/hero.svg 있으면 우선 시도, 실패 시 기존 PNG로 폴백 */
const heroImg = ref("/illustrations/hero.svg");

const architecture = reactive([
  {
    num: "01", en: "Camera / Edge AI", ko: "카메라 / 엣지 AI",
    points: ["다중 카메라 영상 수집", "Edge AI 전처리 및 압축", "네트워크 최적화 전송"],
    svg: "/illustrations/arch-camera.svg", svgBroken: false, lucide: Cctv,
  },
  {
    num: "02", en: "AI Detection Engine", ko: "AI 감지 엔진",
    points: ["차량 감지 및 추적", "번호판 인식 (OCR)", "객체 분류 및 이벤트 검출"],
    svg: "/illustrations/arch-ai.svg", svgBroken: false, lucide: BrainCircuit,
  },
  {
    num: "03", en: "Data Platform", ko: "데이터 플랫폼",
    points: ["실시간 스트리밍 처리", "시계열 데이터 저장", "통계 / 집계 / 분석 엔진"],
    svg: "/illustrations/arch-database.svg", svgBroken: false, lucide: Database,
  },
  {
    num: "04", en: "Dashboard / API", ko: "대시보드 / API",
    points: ["실시간 대시보드", "리포트 및 통계", "외부 시스템 API 연동"],
    svg: "/illustrations/arch-dashboard.svg", svgBroken: false, lucide: LayoutDashboard,
  },
]);
...
const lifecycle = [
  ...
  { title: "분석", desc: "유입/유출, 차량 흐름 등 교통 데이터를 분석합니다.",     lucide: BrainCircuit },
  { title: "저장", desc: "분석 결과를 안전하게 저장하고 관리합니다.",            lucide: Database },
  { title: "활용", desc: "대시보드, 알림, API로 다양한 채널에 제공합니다.",      lucide: LayoutDashboard },
];
...
const kpis = [
  ...
  { title: "운영 효율성",      value: "40%+",    desc: "교통 관리 비용 절감", lucide: TrendingUp },
  { title: "데이터 신뢰성",    value: "99.9%",   desc: "데이터 안정성 보장",  lucide: ShieldCheck },
...
.hero h1 {
  font-family: "Pretendard Variable", Pretendard, sans-serif;
  font-size: clamp(28px, 3vw, 40px);  /* 320px 박스에 맞춰 컴팩트 */
...
.hero-right {
  position: relative;
  /* 상/하 padding(32px) 완전 무효화 + 1px 추가로 헤더 라인에 정확히 붙음
     우측: 화면 끝까지 풀블리드 */
  margin-top: -33px;
  margin-bottom: -33px;
  margin-left: 0;
...
.hero-img {
  ...
  display: block;
  /* 좌측 30%까지 부드러운 그라데이션 — 텍스트와 자연스럽게 어우러짐 */
...
.sec-head p {
  font-size: 15px; color: var(--t); opacity: 0.75;
  margin: 0;
}
...
.theme-navy.light .arch-card { background: #fff; box-shadow: 0 2px 14px rgba(15,40,90,0.04); }
.arch-head {
  display: flex; align-items: flex-start; gap: 12px;
  margin-bottom: 36px;       /* 헤더 ↔ 본문 간격 확장 */
}
.arch-num {
  flex-shrink: 0;
  font-family: "Pretendard Variable", Pretendard, sans-serif;
  font-size: 22px; font-weight: 800;
  color: var(--a); letter-spacing: -0.5px;
  line-height: 1;
  padding-top: 2px;
}
.arch-titles { flex: 1; min-width: 0; }
.arch-en {
  font-size: 16.5px; font-weight: 700; color: var(--t);
  letter-spacing: -0.2px;
  margin-bottom: 4px;
}
.arch-ko {
  font-size: 14px; color: var(--t); opacity: 0.78;
  font-weight: 500;
}
/* 본문: 좌측 아이콘 + 우측 불릿 리스트 — 그룹 전체를 카드 가운데 정렬 */
.arch-body {
  display: flex; align-items: center; justify-content: center;
  gap: 18px;
  margin-top: auto;
...
.arch-svg { width: 100%; max-width: 110px; height: auto; display: block; }
.arch-list {
  list-style: none; padding: 0; margin: 0;
  display: flex; flex-direction: column; gap: 8px;
}
.arch-list li {
  font-size: 13px; color: var(--t); opacity: 0.78;
  font-weight: 500;
  display: flex; align-items: flex-start; gap: 8px;
  line-height: 1.5;
  word-break: keep-all;
}
.arch-list .dot { color: var(--a); font-size: 14px; line-height: 1.35; flex-shrink: 0; }
.arch-arrow {
  display: flex; align-items: center; justify-content: center;
  color: var(--a);
}
...
.theme-navy.light .lc-icon { background: rgba(37,99,235,0.06); }
.lc-title {
  font-size: 18px; font-weight: 700; color: var(--t);
  margin-bottom: 10px;
}
.lc-desc {
  font-size: 13.5px; color: var(--t); opacity: 0.78;
  font-weight: 500;
  line-height: 1.6; max-width: 170px; margin: 0;
  word-break: keep-all;
}
.lc-line {
  flex-shrink: 0;
...
.theme-navy.light .mod-card { background: #fff; box-shadow: 0 2px 14px rgba(15,40,90,0.04); }
.mod-num {
  font-size: 14.5px; font-weight: 700; color: var(--a);
  margin-bottom: 8px;
}
.mod-title {
  font-size: 19px; font-weight: 800; color: var(--t);
...
.mod-desc {
  font-size: 13.5px; color: var(--t); opacity: 0.8;
  font-weight: 500;
  line-height: 1.65; white-space: pre-line; margin: 0 0 16px;
}
.mod-tags { display: flex; flex-wrap: wrap; gap: 7px; justify-content: center; }
.mod-tag {
...
.theme-navy.light .mod-tag { background: rgba(37,99,235,0.08); }
...
.theme-navy.light .deploy-box { background: #fff; box-shadow: 0 2px 14px rgba(15,40,90,0.04); }
.deploy-h {
...
.deploy-desc {
  font-size: 12.5px; color: var(--t); opacity: 0.72;
  font-weight: 500;
}
.deploy-arrow {
  display: flex; align-items: center; justify-content: center;
  color: var(--a); opacity: 0.6;
}
/* 배포 박스는 3개 항목 */
...
.impact-sec::before {
  ...
  -webkit-mask-image: radial-gradient(ellipse 75% 60% at 50% 40%, #000 50%, transparent 100%);
}
.impact-sec > * { position: relative; z-index: 1; }
...
.kpi-card:hover .kpi-value { color: #93c5fd; }
.kpi-icon {
...
.kpi-value { transition: color 0.22s; }
.kpi-title {
...
.kpi-value {
  ...
  letter-spacing: -1.3px; line-height: 1;
  color: #fff;
  margin-bottom: 12px;
}
.kpi-desc {
...
@media (max-width: 1100px) {
  ...
  .lc-row { flex-wrap: wrap; gap: 24px; }
  .lc-line { display: none; }
  .lc-step { flex: 0 0 calc(33% - 16px); }
...
@media (max-width: 768px) {
  ...
  .sec, .impact-sec { padding-left: 24px; padding-right: 24px; }
  .deploy-grid { grid-template-columns: repeat(2, 1fr) !important; }
