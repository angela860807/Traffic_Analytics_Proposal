...
const channels = [
  { key: 'board', tag: 'BOARD',     name: '게시판',     desc: '공지사항, 사용 후기, 활용 사례를 공유합니다.',      badge: '6',    live: false },
  { key: 'qna',   tag: 'Q&A',       name: '질문 & 답변', desc: '기술적인 질문을 올리고 전문 답변을 받습니다.',    badge: '5',    live: false },
  { key: 'chat',  tag: 'LIVE CHAT', name: '실시간 채팅', desc: '운영팀과 WebSocket 기반으로 즉시 소통합니다.', badge: 'Live', live: true  },
]

const active = computed(() => channels.find(c => c.key === tab.value))

onMounted(() => { if (route.query.tab) tab.value = route.query.tab })
...
.sb-item:hover { background: rgba(255,255,255,.03); border-color: var(--ba); }
.sb-item.on {
...
.st-sep { font-size: 10px; color: var(--t3); }
.st-meta {
  font-family: 'IBM Plex Mono', monospace;
...
.st-copy {
  font-family: 'IBM Plex Mono', monospace;
  font-size: 9px; color: var(--t3); opacity: .5;
  line-height: 1.5;
}
...
.ph-name {
  ...
  letter-spacing: -.3px; color: var(--t);
}
.ph-desc {
  font-size: 11px; color: var(--t3);
  font-weight: 300;
...
.panel-body.chat-mode { padding: 0; overflow: hidden; }
.panel-body::-webkit-scrollbar { width: 3px; }
.panel-body::-webkit-scrollbar-thumb { background: var(--b); border-radius: 2px; }

@media (max-width: 768px) {
  .content { height: auto; overflow: auto; }
  .ph { padding: 40px 20px 32px; }
  .layout { flex-direction: column; min-height: 0; overflow: auto; }
  .sidebar { width: 100%; border-right: none; border-bottom: 1px solid var(--b); padding: 20px; }
  .sb-nav { flex-direction: row; overflow-x: auto; gap: 8px; }
  .sb-item { min-width: 160px; }
