...
카메라 영상을 기반으로 차량을 감지하고 번호판을 인식하여, 차량의 유입·유출 흐름을 분석하는 실시간 차량 모니터링 시스템입니다.

본 백엔드는 외부 AI 분석 서버가 전달한 감지 로그와 과속 위반 데이터를 수집하고, OCR 분석 결과, 중복 감지 상태, 차량 흐름 이벤트, 시간대별 교통 통계를 관리하는 Spring Boot API 서버입니다. 또한 회원 인증, 공지사항, 게시글, 댓글, QnA 기능을 위한 REST API를 제공합니다.
...
3. **Storage Layer**
   - PostgreSQL에 원본 감지 로그, 분석 결과, 차량, 카메라, 구역, 과속 위반, 검토 이력, 통계 데이터를 저장합니다.
...
외부 AI 분석 서버가 감지 로그와 과속 위반 데이터를 전송할 때 사용하는 내부 인증 헤더입니다.
